...
public class PasswordForm extends JFrame implements ActionListener {
	...
	GABLogger log = GABLogger.getLogger(PasswordForm.class.getName());
	...
	public PasswordForm() {
        ...
        try{
        	Properties pwds = GABPassword.getAllPasswords();
        	if (pwds!=null){
                if (pwds.containsKey(GABEnvProp.DBPwd)){
                	dbPwd_txt.setText(CipherTextInfo.decrypt(pwds.getProperty(GABEnvProp.DBPwd)));
                } 
                if (pwds.containsKey(GABEnvProp.EmailPwd)){
                	emailPwd_txt.setText(CipherTextInfo.decrypt(pwds.getProperty(GABEnvProp.EmailPwd)));
                } 
                if (pwds.containsKey(GABEnvProp.SmsPwd)){
                	smsPwd_txt.setText(CipherTextInfo.decrypt(pwds.getProperty(GABEnvProp.SmsPwd )));
                } 
        	}
        }catch(Exception er){
	...
	public void actionPerformed(ActionEvent actionEvent) {
		if (actionEvent.getActionCommand().equals("Save")){
			...
			try{
				pwds.put( GABEnvProp.DBPwd, CipherTextInfo.encrypt(String.valueOf(dbPwd_txt.getPassword())));
				pwds.put( GABEnvProp.EmailPwd, CipherTextInfo.encrypt(String.valueOf(emailPwd_txt.getPassword())));
				pwds.put( GABEnvProp.SmsPwd, CipherTextInfo.encrypt(String.valueOf(smsPwd_txt.getPassword())));
				GABPassword.savePasswords(pwds);
			}catch(Exception er){
